...
import { IconBack, IconUsers, IconSettings, IconDelete, IconAdd, IconCrown, IconShield, IconCopy, IconCamera, IconEdit, IconBell, IconBellOff, IconLock, IconGlobe } from './Icons';
...
export default function ChannelSettings({ chatId, onClose, onUpdate }) {
  ...
  return (
    <div style={styles.container}>
      <header style={styles.header}>
        ...
        <span style={styles.headerTitle}>Настройки канала</span>
        <span style={{ width: 40 }} />
      </header>
      ...
      <div style={styles.content}>
        <div style={styles.tabs}>
          ...
          <button
            type="button"
            style={styles.tab(activeTab === 'notifications')}
        ...
        {activeTab === 'info' && (
          <div style={styles.section}>
            <div style={styles.photoContainer}>
              {chat?.photo ? (
                <img src={chat.photo} alt="" style={{ width: '100%', height: '100%', borderRadius: 24, objectFit: 'cover' }} />
              ) : (
                (chat?.name || 'К').charAt(0).toUpperCase()
              )}
              ...
              <>
                ...
                <div style={{ marginTop: 24, borderTop: `1px solid ${theme.border}`, paddingTop: 16 }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 8 }}>
                    <IconGlobe width={18} height={18} style={{ color: theme.textMuted }} />
                    <span style={{ fontSize: 13, color: theme.textMuted }}>
                      {chat?.isPrivate ? 'Приватный канал' : 'Публичный канал'}
                    </span>
                  </div>
                  <p style={{ fontSize: 13, color: theme.textMuted }}>
                    {chat?.members?.length || 0} подписчиков
                  </p>
                </div>
              </>
            )}
          ...
          <div style={styles.section}>
            <div style={{ display: 'flex', gap: 8, marginBottom: 16 }}>
              <input
                ...
                placeholder="Добавить подписчика (@username)"
                value={newMember}
                ...
                <div style={styles.memberInfo}>
                  ...
                  <div style={styles.memberRole}>
                    <span style={styles.roleBadge(member.role)}>
                      ...
                      {member.role === 'admin' ? 'Админ' : member.role === 'moderator' ? 'Модератор' : 'Подписчик'}
                    </span>
                  </div>
                </div>
                {member.role !== 'admin' && (
        ...
        {activeTab === 'permissions' && (
          <div style={styles.section}>
            <p style={{ fontSize: 15, color: theme.textMuted, lineHeight: 1.6, marginBottom: 20 }}>
              Настройте права доступа для участников канала.
            </p>

            <div style={styles.toggleRow}>
              <span style={styles.toggleLabel}>
                <IconLock width={18} height={18} style={{ marginRight: 8, verticalAlign: 'middle', color: theme.textMuted }} />
                Приватный канал
              </span>
              <input type="checkbox" defaultChecked={chat?.isPrivate} style={{ width: 18, height: 18 }} />
            </div>
        ...
        {activeTab === 'notifications' && (
          <div style={styles.section}>
            <p style={{ fontSize: 15, color: theme.textMuted, lineHeight: 1.6, marginBottom: 20 }}>
              Настройте уведомления для канала.
            </p>

            <div style={styles.toggleRow}>
              <span style={styles.toggleLabel}>
                <IconBell width={18} height={18} style={{ marginRight: 8, verticalAlign: 'middle', color: theme.textMuted }} />
            ...
            <div style={styles.toggleRow}>
              <span style={styles.toggleLabel}>
                <IconBellOff width={18} height={18} style={{ marginRight: 8, verticalAlign: 'middle', color: theme.textMuted }} />
                Тихий режим (без звука)
              </span>
      ...
      {showDeleteConfirm && (
        <div style={styles.deleteConfirm}>
          <div style={styles.deleteDialog}>
            <h3 style={styles.deleteDialogTitle}>Удалить канал?</h3>
            <p style={styles.deleteDialogText}>
              Это действие нельзя отменить. Все сообщения и подписчики будут удалены.
            </p>
